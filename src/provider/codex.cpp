...
Codex::Codex(const CodexCredentials& credentials)
    : Provider(credentials.access_token, "gpt-4o"), credentials_(credentials) {}

const char* Codex::name() const {
    return "openai";
}

nlohmann::json Codex::convert_messages(const std::vector<Message>& messages, const std::string& system) {
        ...
        if (msg.role == Role::User) {
            for (const auto& block : msg.content) {
                if (auto* text = std::get_if<TextBlock>(&block)) {
                    result.push_back({{"role", "user"}, {"content", text->text}});
...
nlohmann::json Codex::convert_tools(const std::vector<ToolDefinition>& tools) {
    nlohmann::json result = nlohmann::json::array();
...
ChatResponse Codex::chat(const std::string& prompt) {
    ...
    auto response = cpr::Post(
        ...
        cpr::Header{{"Authorization", "Bearer " + credentials_.access_token}},
    ...
    ChatResponse result;
    result.message = json_response["choices"][0]["message"]["content"].get<std::string>();
    result.model = json_response["model"].get<std::string>();
    result.prompt_tokens = json_response["usage"]["prompt_tokens"].get<int>();
    result.completion_tokens = json_response["usage"]["completion_tokens"].get<int>();
...
EventStream Codex::complete(
    ...
    if (response.status_code != 200) {
        throw std::runtime_error("OpenAI API error: " + std::to_string(response.status_code) + " " + response.text);
    }

    return parse_sse_stream(response.text);
}

EventStream Codex::parse_sse_stream(const std::string& response_text) {
    EventStream events;
